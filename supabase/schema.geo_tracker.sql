...
CREATE SCHEMA IF NOT EXISTS "geo_tracker";


GRANT USAGE ON SCHEMA "geo_tracker" TO "postgres";
GRANT USAGE ON SCHEMA "geo_tracker" TO "anon";
GRANT USAGE ON SCHEMA "geo_tracker" TO "authenticated";
GRANT USAGE ON SCHEMA "geo_tracker" TO "service_role";


-- A handful of upstream migrations (00023, 00022, 00017/18/19) create tables
-- with unqualified names (relying on Supabase Dashboard's default search_path
-- of "$user", public — never reproduced by this script's session, which the
-- earlier SELECT pg_catalog.set_config('search_path', '', false) leaves
-- empty). Session-level search_path fixes every one of those references —
-- and their unqualified FK targets — at once, without hand-patching each.
-- Deliberately omits "public": unqualified names should resolve only against
-- this fork's own schema or extensions, never fall through to another app's
-- public.* objects of the same name (e.g. public.profiles).
SET search_path = geo_tracker, extensions;


COMMENT ON SCHEMA "geo_tracker" IS 'everest-geo-tracker (ansvisor fork) — self-hosted, isolated from other apps in this project';
...
CREATE TYPE "geo_tracker"."user_role" AS ENUM (
    'admin',
    'manager',
    'analyst',
    'agency_partner'
...
ALTER TYPE "geo_tracker"."user_role" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."prompt_results" (
...
ALTER TABLE "geo_tracker"."prompt_results" OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text" DEFAULT NULL::"text") RETURNS SETOF "geo_tracker"."prompt_results"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
  SELECT DISTINCT ON (pr.prompt_id, pr.platform) pr.*
  FROM geo_tracker.prompt_results pr
...
ALTER FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text" DEFAULT NULL::"text", "p_model" "text" DEFAULT NULL::"text", "p_region" "text" DEFAULT NULL::"text", "p_date_from" timestamp with time zone DEFAULT NULL::timestamp with time zone, "p_date_to" timestamp with time zone DEFAULT NULL::timestamp with time zone) RETURNS SETOF "geo_tracker"."prompt_results"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
...
ALTER FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text", "p_model" "text", "p_region" "text", "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "geo_tracker"."handle_new_user"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'geo_tracker'
    AS $$
begin
  insert into geo_tracker.profiles (id, full_name, avatar_url)
  values (
    new.id,
...
ALTER FUNCTION "geo_tracker"."handle_new_user"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "geo_tracker"."handle_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'geo_tracker'
    AS $$
begin
...
ALTER FUNCTION "geo_tracker"."handle_updated_at"() OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."ai_traffic_logs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "url" "text" NOT NULL,
    "referrer" "text",
...
ALTER TABLE "geo_tracker"."ai_traffic_logs" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."brand_domains" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."brand_domains" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."brand_platforms" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "platform" "text" NOT NULL,
...
ALTER TABLE "geo_tracker"."brand_platforms" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."brands" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "geo_tracker"."brands" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."competitors" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "name" "text" NOT NULL,
...
ALTER TABLE "geo_tracker"."competitors" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."content_opportunities" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "prompt_id" "uuid",
    "title" "text" NOT NULL,
...
ALTER TABLE "geo_tracker"."content_opportunities" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."jobs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "type" "text" NOT NULL,
    "brand_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."jobs" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."organizations" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "name" "text" NOT NULL,
    "slug" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "geo_tracker"."organizations" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."profiles" (
    "id" "uuid" NOT NULL,
    "full_name" "text",
    "avatar_url" "text",
    "role" "geo_tracker"."user_role" DEFAULT 'admin'::"geo_tracker"."user_role" NOT NULL,
...
ALTER TABLE "geo_tracker"."profiles" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."prompt_sets" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "name" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "geo_tracker"."prompt_sets" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."prompt_volumes" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "prompt_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."prompt_volumes" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."prompts" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "geo_tracker"."prompts" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."topics" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "name" "text" NOT NULL,
...
ALTER TABLE "geo_tracker"."topics" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."volume_usage" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."volume_usage" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "geo_tracker"."webhook_configs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."webhook_configs" OWNER TO "postgres";


ALTER TABLE ONLY "geo_tracker"."ai_traffic_logs"
    ADD CONSTRAINT "ai_traffic_logs_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."brand_domains"
    ADD CONSTRAINT "brand_domains_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."brand_platforms"
...
ALTER TABLE ONLY "geo_tracker"."brands"
...
ALTER TABLE ONLY "geo_tracker"."competitors"
    ADD CONSTRAINT "competitors_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."content_opportunities"
    ADD CONSTRAINT "content_opportunities_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."jobs"
    ADD CONSTRAINT "jobs_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."organizations"
...
ALTER TABLE ONLY "geo_tracker"."profiles"
    ADD CONSTRAINT "profiles_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."prompt_results"
    ADD CONSTRAINT "prompt_results_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."prompt_sets"
    ADD CONSTRAINT "prompt_sets_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."prompt_volumes"
    ADD CONSTRAINT "prompt_volumes_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."prompts"
    ADD CONSTRAINT "prompts_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."topics"
...
ALTER TABLE ONLY "geo_tracker"."volume_usage"
    ADD CONSTRAINT "volume_usage_pkey" PRIMARY KEY ("id");


ALTER TABLE ONLY "geo_tracker"."webhook_configs"
...
CREATE INDEX "idx_ai_traffic_logs_brand_created" ON "geo_tracker"."ai_traffic_logs" USING "btree" ("brand_id", "created_at" DESC);


CREATE INDEX "idx_ai_traffic_logs_brand_id" ON "geo_tracker"."ai_traffic_logs" USING "btree" ("brand_id");


CREATE INDEX "idx_ai_traffic_logs_created_at" ON "geo_tracker"."ai_traffic_logs" USING "btree" ("created_at" DESC);


CREATE INDEX "idx_ai_traffic_logs_source_platform" ON "geo_tracker"."ai_traffic_logs" USING "btree" ("source_platform");


CREATE INDEX "idx_brand_domains_brand_id" ON "geo_tracker"."brand_domains" USING "btree" ("brand_id");


CREATE INDEX "idx_brand_platforms_brand_id" ON "geo_tracker"."brand_platforms" USING "btree" ("brand_id");


CREATE INDEX "idx_brands_organization_id" ON "geo_tracker"."brands" USING "btree" ("organization_id");


CREATE INDEX "idx_brands_tracking_code" ON "geo_tracker"."brands" USING "btree" ("tracking_code");


CREATE INDEX "idx_co_brand_id" ON "geo_tracker"."content_opportunities" USING "btree" ("brand_id");


CREATE INDEX "idx_co_score" ON "geo_tracker"."content_opportunities" USING "btree" ("opportunity_score" DESC);


CREATE INDEX "idx_co_status" ON "geo_tracker"."content_opportunities" USING "btree" ("status");


CREATE INDEX "idx_competitors_brand_id" ON "geo_tracker"."competitors" USING "btree" ("brand_id");


CREATE INDEX "idx_jobs_brand_id" ON "geo_tracker"."jobs" USING "btree" ("brand_id");


CREATE INDEX "idx_jobs_status" ON "geo_tracker"."jobs" USING "btree" ("status");


CREATE INDEX "idx_jobs_type_status" ON "geo_tracker"."jobs" USING "btree" ("type", "status");


CREATE INDEX "idx_profiles_organization_id" ON "geo_tracker"."profiles" USING "btree" ("organization_id");


CREATE INDEX "idx_prompt_results_brand_id" ON "geo_tracker"."prompt_results" USING "btree" ("brand_id");


CREATE INDEX "idx_prompt_results_created_at" ON "geo_tracker"."prompt_results" USING "btree" ("created_at" DESC);


CREATE INDEX "idx_prompt_results_prompt_id" ON "geo_tracker"."prompt_results" USING "btree" ("prompt_id");


CREATE INDEX "idx_prompt_volumes_est_ai_volume" ON "geo_tracker"."prompt_volumes" USING "btree" ("est_ai_volume" DESC);


CREATE INDEX "idx_prompt_volumes_prompt_id" ON "geo_tracker"."prompt_volumes" USING "btree" ("prompt_id");


CREATE INDEX "idx_prompts_topic" ON "geo_tracker"."prompts" USING "btree" ("topic_id");


CREATE INDEX "idx_topics_brand" ON "geo_tracker"."topics" USING "btree" ("brand_id");


CREATE INDEX "idx_volume_usage_org_month" ON "geo_tracker"."volume_usage" USING "btree" ("organization_id", "used_at");


CREATE INDEX "idx_wc_brand_id" ON "geo_tracker"."webhook_configs" USING "btree" ("brand_id");


CREATE OR REPLACE TRIGGER "handle_prompt_sets_updated_at" BEFORE UPDATE ON "geo_tracker"."prompt_sets" FOR EACH ROW EXECUTE FUNCTION "geo_tracker"."handle_updated_at"();


CREATE OR REPLACE TRIGGER "trg_brands_updated_at" BEFORE UPDATE ON "geo_tracker"."brands" FOR EACH ROW EXECUTE FUNCTION "geo_tracker"."handle_updated_at"();


CREATE OR REPLACE TRIGGER "trg_organizations_updated_at" BEFORE UPDATE ON "geo_tracker"."organizations" FOR EACH ROW EXECUTE FUNCTION "geo_tracker"."handle_updated_at"();


CREATE OR REPLACE TRIGGER "trg_profiles_updated_at" BEFORE UPDATE ON "geo_tracker"."profiles" FOR EACH ROW EXECUTE FUNCTION "geo_tracker"."handle_updated_at"();


ALTER TABLE ONLY "geo_tracker"."ai_traffic_logs"
    ADD CONSTRAINT "ai_traffic_logs_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."brand_domains"
    ADD CONSTRAINT "brand_domains_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."brand_platforms"
    ADD CONSTRAINT "brand_platforms_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."brands"
    ADD CONSTRAINT "brands_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "geo_tracker"."organizations"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."competitors"
    ADD CONSTRAINT "competitors_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."content_opportunities"
    ADD CONSTRAINT "content_opportunities_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."content_opportunities"
    ADD CONSTRAINT "content_opportunities_prompt_id_fkey" FOREIGN KEY ("prompt_id") REFERENCES "geo_tracker"."prompts"("id") ON DELETE SET NULL;


ALTER TABLE ONLY "geo_tracker"."jobs"
    ADD CONSTRAINT "jobs_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "geo_tracker"."profiles"
    ADD CONSTRAINT "profiles_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "geo_tracker"."organizations"("id") ON DELETE SET NULL;


ALTER TABLE ONLY "geo_tracker"."prompt_results"
    ADD CONSTRAINT "prompt_results_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."prompt_results"
    ADD CONSTRAINT "prompt_results_prompt_id_fkey" FOREIGN KEY ("prompt_id") REFERENCES "geo_tracker"."prompts"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."prompt_sets"
    ADD CONSTRAINT "prompt_sets_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."prompt_volumes"
    ADD CONSTRAINT "prompt_volumes_prompt_id_fkey" FOREIGN KEY ("prompt_id") REFERENCES "geo_tracker"."prompts"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."prompts"
    ADD CONSTRAINT "prompts_prompt_set_id_fkey" FOREIGN KEY ("prompt_set_id") REFERENCES "geo_tracker"."prompt_sets"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."prompts"
    ADD CONSTRAINT "prompts_topic_id_fkey" FOREIGN KEY ("topic_id") REFERENCES "geo_tracker"."topics"("id") ON DELETE SET NULL;


ALTER TABLE ONLY "geo_tracker"."topics"
    ADD CONSTRAINT "topics_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."volume_usage"
    ADD CONSTRAINT "volume_usage_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "geo_tracker"."organizations"("id") ON DELETE CASCADE;


ALTER TABLE ONLY "geo_tracker"."webhook_configs"
    ADD CONSTRAINT "webhook_configs_brand_id_fkey" FOREIGN KEY ("brand_id") REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE;


CREATE POLICY "Service role can delete prompt results" ON "geo_tracker"."prompt_results" FOR DELETE USING (true);


CREATE POLICY "Service role can insert prompt results" ON "geo_tracker"."prompt_results" FOR INSERT WITH CHECK (true);


CREATE POLICY "Service role can insert traffic logs" ON "geo_tracker"."ai_traffic_logs" FOR INSERT WITH CHECK (true);


CREATE POLICY "Users can delete brand platforms through org" ON "geo_tracker"."brand_platforms" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("b"."id" = "brand_platforms"."brand_id") AND ("p"."id" = "auth"."uid"())))));


CREATE POLICY "Users can insert brand platforms through org" ON "geo_tracker"."brand_platforms" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("b"."id" = "brand_platforms"."brand_id") AND ("p"."id" = "auth"."uid"())))));


CREATE POLICY "Users can read own org prompt results" ON "geo_tracker"."prompt_results" FOR SELECT USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));


CREATE POLICY "Users can update brand platforms through org" ON "geo_tracker"."brand_platforms" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("b"."id" = "brand_platforms"."brand_id") AND ("p"."id" = "auth"."uid"())))));


CREATE POLICY "Users can view brand platforms through org" ON "geo_tracker"."brand_platforms" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("b"."id" = "brand_platforms"."brand_id") AND ("p"."id" = "auth"."uid"())))));


CREATE POLICY "Users can view own org traffic logs" ON "geo_tracker"."ai_traffic_logs" FOR SELECT USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));


CREATE POLICY "Users cannot update plan fields directly" ON "geo_tracker"."organizations" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "geo_tracker"."profiles"
...
ALTER TABLE "geo_tracker"."ai_traffic_logs" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "geo_tracker"."brand_domains" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "brand_domains: admin/manager delete" ON "geo_tracker"."brand_domains" FOR DELETE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "brand_domains: admin/manager insert" ON "geo_tracker"."brand_domains" FOR INSERT WITH CHECK (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "brand_domains: admin/manager update" ON "geo_tracker"."brand_domains" FOR UPDATE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "brand_domains: member select" ON "geo_tracker"."brand_domains" FOR SELECT USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));


ALTER TABLE "geo_tracker"."brand_platforms" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "geo_tracker"."brands" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "brands: admin delete" ON "geo_tracker"."brands" FOR DELETE USING (("organization_id" IN ( SELECT "profiles"."organization_id"
   FROM "geo_tracker"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'admin'::"geo_tracker"."user_role")))));


CREATE POLICY "brands: admin/manager insert" ON "geo_tracker"."brands" FOR INSERT WITH CHECK (("organization_id" IN ( SELECT "profiles"."organization_id"
   FROM "geo_tracker"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "brands: admin/manager update" ON "geo_tracker"."brands" FOR UPDATE USING (("organization_id" IN ( SELECT "profiles"."organization_id"
   FROM "geo_tracker"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "brands: member select" ON "geo_tracker"."brands" FOR SELECT USING (("organization_id" IN ( SELECT "profiles"."organization_id"
   FROM "geo_tracker"."profiles"
  WHERE ("profiles"."id" = "auth"."uid"()))));


ALTER TABLE "geo_tracker"."content_opportunities" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "content_opportunities: admin/manager delete" ON "geo_tracker"."content_opportunities" FOR DELETE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "content_opportunities: admin/manager insert" ON "geo_tracker"."content_opportunities" FOR INSERT WITH CHECK (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "content_opportunities: admin/manager update" ON "geo_tracker"."content_opportunities" FOR UPDATE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "content_opportunities: member select" ON "geo_tracker"."content_opportunities" FOR SELECT USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));


ALTER TABLE "geo_tracker"."organizations" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "organizations: admin update" ON "geo_tracker"."organizations" FOR UPDATE USING (("id" IN ( SELECT "profiles"."organization_id"
   FROM "geo_tracker"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'admin'::"geo_tracker"."user_role")))));


CREATE POLICY "organizations: authenticated insert" ON "geo_tracker"."organizations" FOR INSERT WITH CHECK (("auth"."uid"() IS NOT NULL));


CREATE POLICY "organizations: member or creator select" ON "geo_tracker"."organizations" FOR SELECT USING ((("id" IN ( SELECT "profiles"."organization_id"
   FROM "geo_tracker"."profiles"
...
ALTER TABLE "geo_tracker"."profiles" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "profiles: own row select" ON "geo_tracker"."profiles" FOR SELECT USING (("auth"."uid"() = "id"));


CREATE POLICY "profiles: own row update" ON "geo_tracker"."profiles" FOR UPDATE USING (("auth"."uid"() = "id"));


ALTER TABLE "geo_tracker"."prompt_results" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "geo_tracker"."prompt_sets" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "prompt_sets: admin/manager delete" ON "geo_tracker"."prompt_sets" FOR DELETE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "prompt_sets: admin/manager insert" ON "geo_tracker"."prompt_sets" FOR INSERT WITH CHECK (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "prompt_sets: admin/manager update" ON "geo_tracker"."prompt_sets" FOR UPDATE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "prompt_sets: member select" ON "geo_tracker"."prompt_sets" FOR SELECT USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));


ALTER TABLE "geo_tracker"."prompts" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "prompts: admin/manager delete" ON "geo_tracker"."prompts" FOR DELETE USING (("prompt_set_id" IN ( SELECT "ps"."id"
   FROM (("geo_tracker"."prompt_sets" "ps"
     JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "prompts: admin/manager insert" ON "geo_tracker"."prompts" FOR INSERT WITH CHECK (("prompt_set_id" IN ( SELECT "ps"."id"
   FROM (("geo_tracker"."prompt_sets" "ps"
     JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));
...
CREATE POLICY "prompts: admin/manager update" ON "geo_tracker"."prompts" FOR UPDATE USING (("prompt_set_id" IN ( SELECT "ps"."id"
   FROM (("geo_tracker"."prompt_sets" "ps"
     JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));
...
CREATE POLICY "prompts: member select" ON "geo_tracker"."prompts" FOR SELECT USING (("prompt_set_id" IN ( SELECT "ps"."id"
   FROM (("geo_tracker"."prompt_sets" "ps"
     JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));
...
ALTER TABLE "geo_tracker"."volume_usage" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "geo_tracker"."webhook_configs" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "webhook_configs: admin/manager delete" ON "geo_tracker"."webhook_configs" FOR DELETE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "webhook_configs: admin/manager insert" ON "geo_tracker"."webhook_configs" FOR INSERT WITH CHECK (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "webhook_configs: admin/manager update" ON "geo_tracker"."webhook_configs" FOR UPDATE USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"]))))));


CREATE POLICY "webhook_configs: member select" ON "geo_tracker"."webhook_configs" FOR SELECT USING (("brand_id" IN ( SELECT "b"."id"
   FROM ("geo_tracker"."brands" "b"
     JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
  WHERE ("p"."id" = "auth"."uid"()))));
...
GRANT ALL ON TABLE "geo_tracker"."prompt_results" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompt_results" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompt_results" TO "service_role";


GRANT ALL ON FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text") TO "anon";
GRANT ALL ON FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text") TO "authenticated";
GRANT ALL ON FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text") TO "service_role";


GRANT ALL ON FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text", "p_model" "text", "p_region" "text", "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone) TO "anon";
GRANT ALL ON FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text", "p_model" "text", "p_region" "text", "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone) TO "authenticated";
GRANT ALL ON FUNCTION "geo_tracker"."get_latest_prompt_results"("p_brand_id" "uuid", "p_platform" "text", "p_model" "text", "p_region" "text", "p_date_from" timestamp with time zone, "p_date_to" timestamp with time zone) TO "service_role";


GRANT ALL ON FUNCTION "geo_tracker"."handle_new_user"() TO "anon";
GRANT ALL ON FUNCTION "geo_tracker"."handle_new_user"() TO "authenticated";
GRANT ALL ON FUNCTION "geo_tracker"."handle_new_user"() TO "service_role";


GRANT ALL ON FUNCTION "geo_tracker"."handle_updated_at"() TO "anon";
GRANT ALL ON FUNCTION "geo_tracker"."handle_updated_at"() TO "authenticated";
GRANT ALL ON FUNCTION "geo_tracker"."handle_updated_at"() TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."ai_traffic_logs" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."ai_traffic_logs" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."ai_traffic_logs" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."brand_domains" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."brand_domains" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."brand_domains" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."brand_platforms" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."brand_platforms" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."brand_platforms" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."brands" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."brands" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."brands" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."competitors" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."competitors" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."competitors" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."content_opportunities" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."content_opportunities" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."content_opportunities" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."jobs" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."jobs" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."jobs" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."organizations" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."organizations" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."organizations" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."profiles" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."profiles" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."profiles" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."prompt_sets" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompt_sets" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompt_sets" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."prompt_volumes" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompt_volumes" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompt_volumes" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."prompts" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompts" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompts" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."topics" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."topics" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."topics" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."volume_usage" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."volume_usage" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."volume_usage" TO "service_role";


GRANT ALL ON TABLE "geo_tracker"."webhook_configs" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."webhook_configs" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."webhook_configs" TO "service_role";


ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON SEQUENCES TO "postgres";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON SEQUENCES TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON SEQUENCES TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON SEQUENCES TO "service_role";


ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON FUNCTIONS TO "postgres";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON FUNCTIONS TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON FUNCTIONS TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON FUNCTIONS TO "service_role";


ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON TABLES TO "postgres";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON TABLES TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON TABLES TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "geo_tracker" GRANT ALL ON TABLES TO "service_role";


CREATE TRIGGER on_auth_user_created_geo_tracker
  AFTER INSERT ON auth.users
  FOR EACH ROW
  EXECUTE FUNCTION geo_tracker.handle_new_user();
...
CREATE TYPE "geo_tracker"."invitation_status" AS ENUM (
    'pending',
    'accepted',
    'expired',
    'revoked'
...
ALTER TYPE "geo_tracker"."invitation_status" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "geo_tracker"."invitations" (
    "id" uuid DEFAULT gen_random_uuid() NOT NULL,
    "organization_id" uuid NOT NULL,
    "email" text NOT NULL,
    "role" geo_tracker.user_role NOT NULL DEFAULT 'analyst',
    "token" text NOT NULL,
    "invited_by" uuid NOT NULL,
    "status" geo_tracker.invitation_status NOT NULL DEFAULT 'pending',
    "expires_at" timestamptz NOT NULL DEFAULT (now() + interval '7 days'),
    "accepted_at" timestamptz,
    "created_at" timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE "geo_tracker"."invitations" OWNER TO "postgres";

ALTER TABLE ONLY "geo_tracker"."invitations"
...
ALTER TABLE ONLY "geo_tracker"."invitations"
    ...
    REFERENCES "geo_tracker"."organizations"("id") ON DELETE CASCADE;
...
CREATE UNIQUE INDEX IF NOT EXISTS "invitations_org_email_pending_idx"
    ON "geo_tracker"."invitations" ("organization_id", lower("email"))
    WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS "idx_invitations_organization_id"
    ON "geo_tracker"."invitations" USING btree ("organization_id");

CREATE INDEX IF NOT EXISTS "idx_invitations_email"
    ON "geo_tracker"."invitations" USING btree (lower("email"));

CREATE INDEX IF NOT EXISTS "idx_invitations_token"
    ON "geo_tracker"."invitations" USING btree ("token");

-- RLS
ALTER TABLE "geo_tracker"."invitations" ENABLE ROW LEVEL SECURITY;

-- Members of an org can read invitations for that org.
CREATE POLICY "Members can view org invitations"
    ON "geo_tracker"."invitations" FOR SELECT
    USING (
        organization_id IN (
            SELECT organization_id FROM geo_tracker.profiles WHERE id = auth.uid()
        )
    );

-- Admins of an org can manage (insert/update/delete) invitations.
CREATE POLICY "Admins can insert invitations"
    ON "geo_tracker"."invitations" FOR INSERT
    WITH CHECK (
        organization_id IN (
            SELECT organization_id FROM geo_tracker.profiles
            WHERE id = auth.uid() AND role = 'admin'
        )
    );

CREATE POLICY "Admins can update invitations"
    ON "geo_tracker"."invitations" FOR UPDATE
    USING (
        organization_id IN (
            SELECT organization_id FROM geo_tracker.profiles
            WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admins can delete invitations"
    ON "geo_tracker"."invitations" FOR DELETE
    USING (
        organization_id IN (
            SELECT organization_id FROM geo_tracker.profiles
            WHERE id = auth.uid() AND role = 'admin'
...
GRANT ALL ON TABLE "geo_tracker"."invitations" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."invitations" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."invitations" TO "service_role";
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."api_keys" (
    "id" uuid DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE "geo_tracker"."api_keys" OWNER TO "postgres";

ALTER TABLE ONLY "geo_tracker"."api_keys"
...
CREATE INDEX IF NOT EXISTS "idx_api_keys_user_id"
    ON "geo_tracker"."api_keys" USING btree ("user_id");

CREATE INDEX IF NOT EXISTS "idx_api_keys_key_hash"
    ON "geo_tracker"."api_keys" USING btree ("key_hash");

-- RLS
ALTER TABLE "geo_tracker"."api_keys" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own api keys"
    ON "geo_tracker"."api_keys" FOR SELECT
    USING (user_id = auth.uid());

CREATE POLICY "Users can insert their own api keys"
    ON "geo_tracker"."api_keys" FOR INSERT
    WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users can revoke their own api keys"
    ON "geo_tracker"."api_keys" FOR UPDATE
    USING (user_id = auth.uid());

CREATE POLICY "Users can delete their own api keys"
    ON "geo_tracker"."api_keys" FOR DELETE
    USING (user_id = auth.uid());

GRANT ALL ON TABLE "geo_tracker"."api_keys" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."api_keys" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."api_keys" TO "service_role";
...
ALTER TABLE "geo_tracker"."prompt_volumes"
...
ALTER TABLE "geo_tracker"."prompt_results"
...
CREATE INDEX IF NOT EXISTS idx_prompt_results_brand_created
  ON geo_tracker.prompt_results USING btree (brand_id, created_at DESC);
...
CREATE OR REPLACE FUNCTION geo_tracker.insights_aggregates(
  p_brand_id   uuid,
  ...
  WITH filtered AS (
    ...
    WHERE pr.brand_id = p_brand_id
      ...
      AND (p_topic_id  IS NULL OR EXISTS (
             SELECT 1 FROM geo_tracker.prompts p
...
CREATE OR REPLACE FUNCTION geo_tracker.competitor_aggregates(
  p_brand_id   uuid,
  p_platform   text         DEFAULT NULL,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
...
CREATE OR REPLACE FUNCTION geo_tracker.share_of_voice_aggregates(
  p_brand_id   uuid,
  p_platform   text         DEFAULT NULL,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
...
GRANT EXECUTE ON FUNCTION geo_tracker.insights_aggregates(uuid, text, text[], text, timestamptz, timestamptz, uuid, uuid)
  TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION geo_tracker.competitor_aggregates(uuid, text, text[], text, timestamptz, timestamptz, uuid, uuid)
  TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION geo_tracker.share_of_voice_aggregates(uuid, text, text[], text, timestamptz, timestamptz, uuid, uuid)
  TO authenticated, service_role;
...
-- side.
CREATE OR REPLACE FUNCTION geo_tracker.visibility_trend_aggregates(
  p_brand_id     uuid,
  p_models       text[]       DEFAULT NULL,
  p_region       text         DEFAULT NULL,
  p_date_from    timestamptz  DEFAULT NULL,
...
GRANT EXECUTE ON FUNCTION
  geo_tracker.visibility_trend_aggregates(uuid, text[], text, timestamptz, timestamptz, uuid, text)
  TO authenticated, service_role;
...
-- ── agent_conversations ──────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS geo_tracker.agent_conversations (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  organization_id uuid NOT NULL REFERENCES geo_tracker.organizations(id) ON DELETE CASCADE,
  ...
  brand_id        uuid REFERENCES geo_tracker.brands(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_agent_conversations_user_updated
  ON geo_tracker.agent_conversations (user_id, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS geo_tracker.agent_messages (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id uuid NOT NULL REFERENCES geo_tracker.agent_conversations(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_agent_messages_conv_created
  ON geo_tracker.agent_messages (conversation_id, created_at);
...
CREATE TABLE IF NOT EXISTS geo_tracker.agent_token_usage (
  id                 uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id            uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  organization_id    uuid NOT NULL REFERENCES geo_tracker.organizations(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_agent_token_usage_user_month
  ON geo_tracker.agent_token_usage (user_id, year_month);


-- ── RLS ────────────────────────────────────────────────────────────────
ALTER TABLE geo_tracker.agent_conversations ENABLE ROW LEVEL SECURITY;
ALTER TABLE geo_tracker.agent_messages      ENABLE ROW LEVEL SECURITY;
ALTER TABLE geo_tracker.agent_token_usage   ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users read own conversations" ON geo_tracker.agent_conversations
  FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users write own conversations" ON geo_tracker.agent_conversations
  FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users read own messages" ON geo_tracker.agent_messages
  FOR SELECT USING (conversation_id IN (
    SELECT id FROM geo_tracker.agent_conversations WHERE user_id = auth.uid()
  ));
CREATE POLICY "Users write own messages" ON geo_tracker.agent_messages
...
CREATE POLICY "Users read own usage" ON geo_tracker.agent_token_usage
  FOR SELECT USING (user_id = auth.uid());
...
CREATE OR REPLACE FUNCTION geo_tracker.touch_agent_conversation_updated_at()
...
CREATE TRIGGER trg_agent_conversations_touch_updated_at
  BEFORE UPDATE ON geo_tracker.agent_conversations
  FOR EACH ROW EXECUTE FUNCTION geo_tracker.touch_agent_conversation_updated_at();

CREATE OR REPLACE FUNCTION geo_tracker.touch_agent_token_usage_updated_at()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER trg_agent_token_usage_touch_updated_at
  BEFORE UPDATE ON geo_tracker.agent_token_usage
  FOR EACH ROW EXECUTE FUNCTION geo_tracker.touch_agent_token_usage_updated_at();
...
alter table geo_tracker.organizations
  ...
  add column if not exists anthropic_api_key_set_by uuid references geo_tracker.profiles(id) on delete set null;

comment on column geo_tracker.organizations.anthropic_api_key_encrypted is
  'AES-256-GCM ciphertext (JSON envelope) of the org-level Anthropic API key. Null = no key configured.';
comment on column geo_tracker.organizations.anthropic_api_key_last4 is
  'Last 4 chars of the plaintext key. Display-only; safe to expose to org members.';
comment on column geo_tracker.organizations.anthropic_api_key_set_at is
  'When the current key was last saved.';
comment on column geo_tracker.organizations.anthropic_api_key_set_by is
...
create table if not exists geo_tracker.prompt_result_shopping_cards (
  id uuid primary key default gen_random_uuid(),
  prompt_result_id uuid not null references geo_tracker.prompt_results(id) on delete cascade,
  -- denormalized from prompt_results.brand_id so org-scoped queries don't
  -- have to join the parent row.
  brand_id uuid not null references geo_tracker.brands(id) on delete cascade,
...
create index if not exists prompt_result_shopping_cards_brand_role_idx
  on geo_tracker.prompt_result_shopping_cards (brand_id, matched_brand_role);
create index if not exists prompt_result_shopping_cards_product_brand_idx
  on geo_tracker.prompt_result_shopping_cards (product_brand);
create index if not exists prompt_result_shopping_cards_merchant_domain_idx
  on geo_tracker.prompt_result_shopping_cards (merchant_domain);

alter table geo_tracker.prompt_result_shopping_cards enable row level security;
...
create policy "shopping_cards: org member select"
  on geo_tracker.prompt_result_shopping_cards
  for select
  using (
    brand_id in (
      select b.id
...
comment on table geo_tracker.prompt_result_shopping_cards is
...
comment on column geo_tracker.prompt_result_shopping_cards.matched_brand_id is
...
comment on column geo_tracker.prompt_result_shopping_cards.raw is
...
ALTER TABLE geo_tracker.brands
...
CREATE OR REPLACE FUNCTION geo_tracker.prompt_performance_aggregates(
  p_brand_id   uuid,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
  p_date_from  timestamptz  DEFAULT NULL,
...
AS $$
  ...
  SELECT COALESCE(
    (SELECT jsonb_agg(
     ...
     FROM aggregated a
     JOIN geo_tracker.prompts p ON p.id = a.prompt_id
     LEFT JOIN geo_tracker.topics t ON t.id = p.topic_id),
    '[]'::jsonb);
$$;

GRANT EXECUTE ON FUNCTION geo_tracker.prompt_performance_aggregates(uuid, text[], text, timestamptz, timestamptz, uuid)
  TO authenticated, service_role;
...
ALTER FUNCTION geo_tracker.get_latest_prompt_results(
  p_brand_id uuid,
  p_platform text
) SECURITY INVOKER;
...
ALTER FUNCTION geo_tracker.insights_aggregates(
  p_brand_id uuid,
  p_platform text,
  p_models text[],
  p_region text,
...
ALTER FUNCTION geo_tracker.competitor_aggregates(
  p_brand_id uuid,
  p_platform text,
  p_models text[],
  p_region text,
...
ALTER FUNCTION geo_tracker.share_of_voice_aggregates(
  p_brand_id uuid,
  p_platform text,
  p_models text[],
  p_region text,
...
ALTER FUNCTION geo_tracker.visibility_trend_aggregates(
  p_brand_id uuid,
  p_models text[],
  p_region text,
  p_date_from timestamptz,
...
ALTER FUNCTION geo_tracker.prompt_performance_aggregates(
  p_brand_id uuid,
  p_models text[],
  p_region text,
  p_date_from timestamptz,
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."brief_usage" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "opportunity_id" "uuid",
    "used_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE "geo_tracker"."brief_usage" OWNER TO "postgres";

ALTER TABLE ONLY "geo_tracker"."brief_usage"
    ADD CONSTRAINT "brief_usage_pkey" PRIMARY KEY ("id");

ALTER TABLE ONLY "geo_tracker"."brief_usage"
    ADD CONSTRAINT "brief_usage_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "geo_tracker"."organizations"("id") ON DELETE CASCADE;

ALTER TABLE ONLY "geo_tracker"."brief_usage"
    ADD CONSTRAINT "brief_usage_opportunity_id_fkey" FOREIGN KEY ("opportunity_id") REFERENCES "geo_tracker"."content_opportunities"("id") ON DELETE SET NULL;

CREATE INDEX "idx_brief_usage_org_month" ON "geo_tracker"."brief_usage" USING "btree" ("organization_id", "used_at");

-- RLS enabled with no policies: only the service role (aeo-server) writes
-- and reads usage rows — same posture as volume_usage.
ALTER TABLE "geo_tracker"."brief_usage" ENABLE ROW LEVEL SECURITY;

GRANT ALL ON TABLE "geo_tracker"."brief_usage" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."brief_usage" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."brief_usage" TO "service_role";
...
--
--   geo_tracker.competitors, geo_tracker.topics, geo_tracker.prompt_volumes, geo_tracker.jobs
--
...
-- ---------------------------------------------------------------------------
ALTER TABLE geo_tracker.jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE geo_tracker.prompt_volumes ENABLE ROW LEVEL SECURITY;

-- ---------------------------------------------------------------------------
-- competitors: org-membership-scoped policies (brand_id -> brands -> profiles).
-- ---------------------------------------------------------------------------
ALTER TABLE geo_tracker.competitors ENABLE ROW LEVEL SECURITY;

CREATE POLICY "competitors: member select" ON geo_tracker.competitors
  FOR SELECT USING (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
CREATE POLICY "competitors: member insert" ON geo_tracker.competitors
...
CREATE POLICY "competitors: member update" ON geo_tracker.competitors
  FOR UPDATE USING (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
CREATE POLICY "competitors: member delete" ON geo_tracker.competitors
  FOR DELETE USING (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
-- ---------------------------------------------------------------------------
ALTER TABLE geo_tracker.topics ENABLE ROW LEVEL SECURITY;

CREATE POLICY "topics: member select" ON geo_tracker.topics
  FOR SELECT USING (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
CREATE POLICY "topics: member insert" ON geo_tracker.topics
  FOR INSERT WITH CHECK (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
CREATE POLICY "topics: member update" ON geo_tracker.topics
  FOR UPDATE USING (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
CREATE POLICY "topics: member delete" ON geo_tracker.topics
  FOR DELETE USING (
    brand_id IN (
      SELECT b.id
      FROM geo_tracker.brands b
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."site_audit_usage" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "organization_id" "uuid" NOT NULL,
    "audit_id" "uuid",
    "used_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
ALTER TABLE ONLY "geo_tracker"."site_audit_usage"
    ADD CONSTRAINT "site_audit_usage_pkey" PRIMARY KEY ("id");

ALTER TABLE ONLY "geo_tracker"."site_audit_usage"
    ADD CONSTRAINT "site_audit_usage_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "geo_tracker"."organizations"("id") ON DELETE CASCADE;

ALTER TABLE ONLY "geo_tracker"."site_audit_usage"
    ADD CONSTRAINT "site_audit_usage_audit_id_fkey" FOREIGN KEY ("audit_id") REFERENCES "geo_tracker"."site_audits"("id") ON DELETE SET NULL;

CREATE INDEX "idx_site_audit_usage_org_month" ON "geo_tracker"."site_audit_usage" USING "btree" ("organization_id", "used_at");
...
ALTER TABLE "geo_tracker"."site_audit_usage" ENABLE ROW LEVEL SECURITY;

GRANT ALL ON TABLE "geo_tracker"."site_audit_usage" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."site_audit_usage" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."site_audit_usage" TO "service_role";
...
CREATE OR REPLACE FUNCTION geo_tracker.tracked_prompt_count(
  p_brand_id   uuid,
  p_platform   text         DEFAULT NULL,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
...
GRANT EXECUTE ON FUNCTION geo_tracker.tracked_prompt_count(
...
CREATE OR REPLACE FUNCTION geo_tracker.prompt_visibility_summaries(
  p_brand_id  uuid,
  p_date_from timestamptz DEFAULT NULL
)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION geo_tracker.prompt_visibility_summaries(uuid, timestamptz)
  TO authenticated;
...
CREATE OR REPLACE FUNCTION geo_tracker.insights_filter_options(p_brand_id uuid)
RETURNS TABLE (
  regions text[],
  models  text[]
)
...
GRANT EXECUTE ON FUNCTION geo_tracker.insights_filter_options(uuid)
  TO authenticated;
...
CREATE OR REPLACE FUNCTION geo_tracker.visible_prompt_stats(
  p_brand_id   uuid,
  p_platform   text         DEFAULT NULL,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
...
GRANT EXECUTE ON FUNCTION geo_tracker.visible_prompt_stats(
  uuid, text, text[], text, timestamptz, timestamptz, uuid
) TO authenticated;
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."topic_suggestions" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    "name" "text" NOT NULL,
    "reason" "text",
    ...
    CONSTRAINT "topic_suggestions_brand_id_fkey" FOREIGN KEY ("brand_id")
        REFERENCES "geo_tracker"."brands"("id") ON DELETE CASCADE,
    CONSTRAINT "topic_suggestions_added_topic_id_fkey" FOREIGN KEY ("added_topic_id")
        REFERENCES "geo_tracker"."topics"("id") ON DELETE SET NULL
);

ALTER TABLE "geo_tracker"."topic_suggestions" OWNER TO "postgres";

CREATE INDEX IF NOT EXISTS "idx_topic_suggestions_brand_status"
    ON "geo_tracker"."topic_suggestions" ("brand_id", "status");

ALTER TABLE "geo_tracker"."topic_suggestions" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "topic_suggestions: member select" ON "geo_tracker"."topic_suggestions"
    FOR SELECT USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
...
CREATE POLICY "topic_suggestions: admin/manager/analyst update" ON "geo_tracker"."topic_suggestions"
    FOR UPDATE USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
            JOIN "geo_tracker"."profiles" "p" ON "p"."organization_id" = "b"."organization_id"
            WHERE "p"."id" = "auth"."uid"()
              AND "p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role", 'analyst'::"geo_tracker"."user_role"])
        )
    );

GRANT ALL ON TABLE "geo_tracker"."topic_suggestions" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."topic_suggestions" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."topic_suggestions" TO "service_role";
...
ALTER TABLE "geo_tracker"."prompts"
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."prompt_notes" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "prompt_id" "uuid" NOT NULL,
    "author_id" "uuid",
    "body" "text" NOT NULL,
    ...
    CONSTRAINT "prompt_notes_prompt_id_fkey" FOREIGN KEY ("prompt_id")
        REFERENCES "geo_tracker"."prompts"("id") ON DELETE CASCADE,
    CONSTRAINT "prompt_notes_author_id_fkey" FOREIGN KEY ("author_id")
        REFERENCES "geo_tracker"."profiles"("id") ON DELETE SET NULL
);

ALTER TABLE "geo_tracker"."prompt_notes" OWNER TO "postgres";

CREATE INDEX IF NOT EXISTS "idx_prompt_notes_prompt"
    ON "geo_tracker"."prompt_notes" ("prompt_id", "created_at");

CREATE TABLE IF NOT EXISTS "geo_tracker"."prompt_target_urls" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "prompt_id" "uuid" NOT NULL,
    "url" "text" NOT NULL,
    "label" "text",
...
ALTER TABLE "geo_tracker"."prompt_target_urls" OWNER TO "postgres";

CREATE INDEX IF NOT EXISTS "idx_prompt_target_urls_prompt"
    ON "geo_tracker"."prompt_target_urls" ("prompt_id", "created_at");

ALTER TABLE "geo_tracker"."prompt_notes" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "geo_tracker"."prompt_target_urls" ENABLE ROW LEVEL SECURITY;

-- Same org scoping as prompts: members read, admin/manager write.
CREATE POLICY "prompt_notes: member select" ON "geo_tracker"."prompt_notes"
    FOR SELECT USING (("prompt_id" IN ( SELECT "pr"."id"
        FROM ((("geo_tracker"."prompts" "pr"
          JOIN "geo_tracker"."prompt_sets" "ps" ON (("ps"."id" = "pr"."prompt_set_id")))
          JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
          JOIN "geo_tracker"."profiles" "p" ON (("p"."organization_id" = "b"."organization_id")))
        WHERE ("p"."id" = "auth"."uid"()))));

CREATE POLICY "prompt_notes: admin/manager insert" ON "geo_tracker"."prompt_notes"
...
CREATE POLICY "prompt_notes: admin/manager delete" ON "geo_tracker"."prompt_notes"
...
CREATE POLICY "prompt_target_urls: member select" ON "geo_tracker"."prompt_target_urls"
    FOR SELECT USING (("prompt_id" IN ( SELECT "pr"."id"
        FROM ((("geo_tracker"."prompts" "pr"
          JOIN "geo_tracker"."prompt_sets" "ps" ON (("ps"."id" = "pr"."prompt_set_id")))
          JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
...
CREATE POLICY "prompt_target_urls: admin/manager insert" ON "geo_tracker"."prompt_target_urls"
    FOR INSERT WITH CHECK (("prompt_id" IN ( SELECT "pr"."id"
        FROM ((("geo_tracker"."prompts" "pr"
          JOIN "geo_tracker"."prompt_sets" "ps" ON (("ps"."id" = "pr"."prompt_set_id")))
          JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
...
CREATE POLICY "prompt_target_urls: admin/manager delete" ON "geo_tracker"."prompt_target_urls"
    FOR DELETE USING (("prompt_id" IN ( SELECT "pr"."id"
        FROM ((("geo_tracker"."prompts" "pr"
          JOIN "geo_tracker"."prompt_sets" "ps" ON (("ps"."id" = "pr"."prompt_set_id")))
          JOIN "geo_tracker"."brands" "b" ON (("b"."id" = "ps"."brand_id")))
...
GRANT ALL ON TABLE "geo_tracker"."prompt_notes" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompt_notes" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompt_notes" TO "service_role";
GRANT ALL ON TABLE "geo_tracker"."prompt_target_urls" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompt_target_urls" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompt_target_urls" TO "service_role";
...
ALTER TABLE "geo_tracker"."prompt_target_urls"
...
DROP FUNCTION geo_tracker.prompt_visibility_summaries(uuid, timestamptz);

CREATE FUNCTION geo_tracker.prompt_visibility_summaries(
  p_brand_id  uuid,
  p_date_from timestamptz DEFAULT NULL
)
RETURNS TABLE (
...
AS $$
  ...
  mentions_flat AS (
    ...
    WHERE cm.value ? 'competitor_id'
      AND EXISTS (
        SELECT 1 FROM geo_tracker.competitors c
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."pulse_settings" (
    "brand_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."pulse_settings" OWNER TO "postgres";

CREATE TABLE IF NOT EXISTS "geo_tracker"."sent_pulses" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
...
ALTER TABLE "geo_tracker"."sent_pulses" OWNER TO "postgres";

CREATE INDEX IF NOT EXISTS "idx_sent_pulses_brand_created"
    ON "geo_tracker"."sent_pulses" ("brand_id", "created_at" DESC);

ALTER TABLE "geo_tracker"."pulse_settings" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "geo_tracker"."sent_pulses" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "pulse_settings: member select" ON "geo_tracker"."pulse_settings"
    FOR SELECT USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
...
CREATE POLICY "pulse_settings: admin/manager insert" ON "geo_tracker"."pulse_settings"
    FOR INSERT WITH CHECK (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
            JOIN "geo_tracker"."profiles" "p" ON "p"."organization_id" = "b"."organization_id"
            WHERE "p"."id" = "auth"."uid"()
              AND "p"."role" = ANY (ARRAY['admin'::"geo_tracker"."user_role", 'manager'::"geo_tracker"."user_role"])
        )
    );

CREATE POLICY "pulse_settings: admin/manager update" ON "geo_tracker"."pulse_settings"
    FOR UPDATE USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
...
CREATE POLICY "sent_pulses: member select" ON "geo_tracker"."sent_pulses"
    FOR SELECT USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
...
GRANT ALL ON TABLE "geo_tracker"."pulse_settings" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."pulse_settings" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."pulse_settings" TO "service_role";

GRANT ALL ON TABLE "geo_tracker"."sent_pulses" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."sent_pulses" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."sent_pulses" TO "service_role";
...
CREATE OR REPLACE FUNCTION geo_tracker.visibility_rate_trend(
  p_brand_id   uuid,
  p_platform   text         DEFAULT NULL,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
...
GRANT EXECUTE ON FUNCTION geo_tracker.visibility_rate_trend(uuid, text, text[], text, timestamptz, timestamptz, uuid)
  TO authenticated;
...
CREATE OR REPLACE FUNCTION geo_tracker.ai_visibility_aggregates(
  p_brand_id   uuid,
  p_platform   text         DEFAULT NULL,
  p_models     text[]       DEFAULT NULL,
  p_region     text         DEFAULT NULL,
...
GRANT EXECUTE ON FUNCTION geo_tracker.ai_visibility_aggregates(uuid, text, text[], text, timestamptz, timestamptz, uuid, uuid)
  TO authenticated;
...
GRANT EXECUTE ON FUNCTION geo_tracker.prompt_visibility_summaries(uuid, timestamptz, timestamptz)
  TO authenticated;
...
CREATE TABLE geo_tracker.tracking_runs (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
...
CREATE INDEX tracking_runs_brand_completed_idx
    ON geo_tracker.tracking_runs (brand_id, completed_at DESC NULLS LAST);
CREATE INDEX tracking_runs_brand_started_idx
    ON geo_tracker.tracking_runs (brand_id, started_at DESC);

ALTER TABLE geo_tracker.tracking_runs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "tracking_runs: member select" ON geo_tracker.tracking_runs
    FOR SELECT USING (
        brand_id IN (
            SELECT b.id
            FROM geo_tracker.brands b
...
GRANT SELECT ON geo_tracker.tracking_runs TO authenticated;
...
WITH latest AS (
    ...
    FROM geo_tracker.prompt_results
    GROUP BY brand_id
)
INSERT INTO geo_tracker.tracking_runs (brand_id, source, started_at, completed_at, result_count)
SELECT l.brand_id,
       'backfill',
...
CREATE TABLE geo_tracker.integration_connections (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id uuid NOT NULL REFERENCES geo_tracker.organizations(id) ON DELETE CASCADE,
    ...
    connected_by uuid REFERENCES geo_tracker.profiles(id) ON DELETE SET NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (organization_id, provider)
);

ALTER TABLE geo_tracker.integration_connections ENABLE ROW LEVEL SECURITY;

CREATE POLICY "integration_connections: member select" ON geo_tracker.integration_connections
    FOR SELECT USING (
        organization_id IN (
            SELECT p.organization_id FROM geo_tracker.profiles p WHERE p.id = auth.uid()
        )
    );

CREATE POLICY "integration_connections: admin write" ON geo_tracker.integration_connections
    FOR ALL USING (
        organization_id IN (
            SELECT p.organization_id FROM geo_tracker.profiles p
            WHERE p.id = auth.uid()
              AND p.role = ANY (ARRAY['admin'::geo_tracker.user_role, 'manager'::geo_tracker.user_role])
        )
    );

GRANT SELECT ON geo_tracker.integration_connections TO authenticated;
...
ALTER TABLE geo_tracker.brands ADD COLUMN gsc_property text;
...
CREATE TABLE geo_tracker.gsc_query_stats (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
    query text NOT NULL,
    date date NOT NULL,
...
CREATE INDEX gsc_query_stats_brand_date_idx
    ON geo_tracker.gsc_query_stats (brand_id, date DESC);

ALTER TABLE geo_tracker.gsc_query_stats ENABLE ROW LEVEL SECURITY;

CREATE POLICY "gsc_query_stats: member select" ON geo_tracker.gsc_query_stats
    FOR SELECT USING (
        brand_id IN (
            SELECT b.id
            FROM geo_tracker.brands b
...
GRANT SELECT ON geo_tracker.gsc_query_stats TO authenticated;
...
CREATE TABLE IF NOT EXISTS "geo_tracker"."prompt_suggestions" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "brand_id" "uuid" NOT NULL,
    ...
    CONSTRAINT "prompt_suggestions_added_prompt_id_fkey" FOREIGN KEY ("added_prompt_id")
        REFERENCES "geo_tracker"."prompts"("id") ON DELETE SET NULL
);

ALTER TABLE "geo_tracker"."prompt_suggestions" OWNER TO "postgres";

CREATE INDEX IF NOT EXISTS "idx_prompt_suggestions_brand_status"
    ON "geo_tracker"."prompt_suggestions" ("brand_id", "status", "generated_at" DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS "idx_prompt_suggestions_brand_text_active"
    ON "geo_tracker"."prompt_suggestions" ("brand_id", "lower"("suggested_text"))
    WHERE ("status" = 'new'::"text");

ALTER TABLE "geo_tracker"."prompt_suggestions" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "prompt_suggestions: member select" ON "geo_tracker"."prompt_suggestions"
    FOR SELECT USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
...
CREATE POLICY "prompt_suggestions: admin/manager/analyst update" ON "geo_tracker"."prompt_suggestions"
    FOR UPDATE USING (
        "brand_id" IN (
            SELECT "b"."id"
            FROM "geo_tracker"."brands" "b"
...
GRANT ALL ON TABLE "geo_tracker"."prompt_suggestions" TO "anon";
GRANT ALL ON TABLE "geo_tracker"."prompt_suggestions" TO "authenticated";
GRANT ALL ON TABLE "geo_tracker"."prompt_suggestions" TO "service_role";

-- ─────────────────────────────────────────────────────────────────────────────

ALTER TABLE geo_tracker.prompt_suggestions ADD COLUMN source_data jsonb;

-- The live table's source check predates 'gsc' — extend it.
ALTER TABLE geo_tracker.prompt_suggestions DROP CONSTRAINT prompt_suggestions_source_check;
ALTER TABLE geo_tracker.prompt_suggestions ADD CONSTRAINT prompt_suggestions_source_check
    CHECK (source = ANY (ARRAY['llm'::text, 'heuristic'::text, 'gsc'::text]));

CREATE FUNCTION geo_tracker.gsc_candidate_queries(
    p_brand_id uuid,
    p_since date,
...
AS $$
    ...
    FROM geo_tracker.gsc_query_stats s
...
CREATE TABLE geo_tracker.dataforseo_competition_cache (
...
ALTER TABLE geo_tracker.dataforseo_competition_cache ENABLE ROW LEVEL SECURITY;
...
--
--   PGRST205: Could not find the table 'geo_tracker.cloro_pending_tasks'
--
...
CREATE TABLE IF NOT EXISTS geo_tracker.cloro_pending_tasks (
    task_id text PRIMARY KEY,
    prompt_id uuid NOT NULL REFERENCES geo_tracker.prompts(id) ON DELETE CASCADE,
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_cloro_pending_tasks_brand_id
    ON geo_tracker.cloro_pending_tasks (brand_id);
CREATE INDEX IF NOT EXISTS idx_cloro_pending_tasks_submitted_at
    ON geo_tracker.cloro_pending_tasks (submitted_at);
...
ALTER TABLE geo_tracker.cloro_pending_tasks ENABLE ROW LEVEL SECURITY;

-- ─────────────────────────────────────────────────────────────────────────
-- migrations/00050_content_opportunity_aggregates.sql
-- ─────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION geo_tracker.content_opportunity_aggregates(
  p_brand_id uuid,
...
AS $$
  ...
  FROM geo_tracker.content_opportunities co
...
GRANT EXECUTE ON FUNCTION geo_tracker.content_opportunity_aggregates(
  uuid,
  text,
  text,
  text,
...
ALTER TABLE geo_tracker.brands ADD COLUMN ga_property_id text;
...
CREATE UNIQUE INDEX IF NOT EXISTS sent_pulses_brand_window_key
    ON geo_tracker.sent_pulses (brand_id, ((payload -> 'window' ->> 'to')))
...
CREATE TABLE geo_tracker.ga_ai_traffic_stats (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
    date date NOT NULL,
...
CREATE INDEX ga_ai_traffic_stats_brand_page_idx
    ON geo_tracker.ga_ai_traffic_stats (brand_id, landing_page);

CREATE INDEX ga_ai_traffic_stats_brand_date_idx
    ON geo_tracker.ga_ai_traffic_stats (brand_id, date DESC);
...
CREATE TABLE geo_tracker.ga_page_stats (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
    date date NOT NULL,
    landing_page text NOT NULL DEFAULT '',
...
CREATE INDEX ga_page_stats_brand_date_idx
    ON geo_tracker.ga_page_stats (brand_id, date DESC);
...
CREATE TABLE geo_tracker.ga_item_stats (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
    date date NOT NULL,
...
CREATE INDEX ga_item_stats_brand_date_idx
    ON geo_tracker.ga_item_stats (brand_id, date DESC);

-- ── RLS ─────────────────────────────────────────────────────────────────────

ALTER TABLE geo_tracker.ga_ai_traffic_stats ENABLE ROW LEVEL SECURITY;
ALTER TABLE geo_tracker.ga_page_stats ENABLE ROW LEVEL SECURITY;
ALTER TABLE geo_tracker.ga_item_stats ENABLE ROW LEVEL SECURITY;

CREATE POLICY "ga_ai_traffic_stats: member select" ON geo_tracker.ga_ai_traffic_stats
    FOR SELECT USING (
        brand_id IN (
            SELECT b.id
            FROM geo_tracker.brands b
...
CREATE POLICY "ga_page_stats: member select" ON geo_tracker.ga_page_stats
    FOR SELECT USING (
        brand_id IN (
            SELECT b.id
            FROM geo_tracker.brands b
...
CREATE POLICY "ga_item_stats: member select" ON geo_tracker.ga_item_stats
    FOR SELECT USING (
        brand_id IN (
            SELECT b.id
            FROM geo_tracker.brands b
...
GRANT SELECT ON geo_tracker.ga_ai_traffic_stats TO authenticated;
GRANT SELECT ON geo_tracker.ga_page_stats TO authenticated;
GRANT SELECT ON geo_tracker.ga_item_stats TO authenticated;
...
DROP FUNCTION IF EXISTS geo_tracker.prompt_visibility_summaries(uuid, timestamptz);
DROP FUNCTION IF EXISTS geo_tracker.prompt_visibility_summaries(uuid, timestamptz, timestamptz);
...
CREATE TABLE geo_tracker.page_opportunities (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    brand_id uuid NOT NULL REFERENCES geo_tracker.brands(id) ON DELETE CASCADE,
...
CREATE INDEX page_opportunities_brand_open_idx
    ON geo_tracker.page_opportunities (brand_id, value_percentile DESC)
    WHERE resolved_at IS NULL;

ALTER TABLE geo_tracker.page_opportunities ENABLE ROW LEVEL SECURITY;

CREATE POLICY "page_opportunities: member select" ON geo_tracker.page_opportunities
    FOR SELECT USING (
        brand_id IN (
            SELECT b.id
            FROM geo_tracker.brands b
...
GRANT SELECT ON geo_tracker.page_opportunities TO authenticated;
...
CREATE FUNCTION geo_tracker.topics_overview_aggregates(p_brand_id uuid)
RETURNS TABLE (
  topic_id             uuid,
  answers              bigint,
  mention_answers      bigint,
...
rows AS (
  ...
  FROM geo_tracker.prompt_results pr
  JOIN geo_tracker.prompts p        ON p.id = pr.prompt_id
  JOIN geo_tracker.prompt_sets ps   ON ps.id = p.prompt_set_id
  CROSS JOIN bounds b
  WHERE pr.brand_id = p_brand_id
...
GRANT EXECUTE ON FUNCTION geo_tracker.topics_overview_aggregates(uuid) TO authenticated;
...
create table if not exists geo_tracker.citation_urls (
...
create unique index if not exists citation_urls_url_key on geo_tracker.citation_urls (url);
create index if not exists citation_urls_domain_idx on geo_tracker.citation_urls (domain);

-- ─── Facts ───────────────────────────────────────────────────────────────────

create table if not exists geo_tracker.prompt_result_citations (
  prompt_result_id uuid not null references geo_tracker.prompt_results(id) on delete cascade,
  ...
  position integer not null,
  url_id bigint not null references geo_tracker.citation_urls(id),
...
create index if not exists prompt_result_citations_brand_created_idx
  on geo_tracker.prompt_result_citations (brand_id, created_at desc);

-- Reverse lookup: which answers cited this URL.
create index if not exists prompt_result_citations_url_idx
  on geo_tracker.prompt_result_citations (url_id);

-- ─── Row level security ──────────────────────────────────────────────────────

alter table geo_tracker.prompt_result_citations enable row level security;
...
create policy "citations: org member select"
  on geo_tracker.prompt_result_citations
  for select
  using (
    brand_id in (
      select b.id
...
-- every customer.
alter table geo_tracker.citation_urls enable row level security;

create policy "Service role can insert citation urls"
  on geo_tracker.citation_urls
  for insert
  with check (true);

comment on table geo_tracker.prompt_result_citations is
...
comment on table geo_tracker.citation_urls is
...
comment on column geo_tracker.prompt_result_citations.position is
...
create or replace function geo_tracker.citation_url_ids(p_urls jsonb)
...
begin
  ...
  insert into geo_tracker.citation_urls (url, domain, title)
  ...
  return query
  select cu.id, cu.url
  from geo_tracker.citation_urls cu
  join (
...
revoke all on function geo_tracker.citation_url_ids(jsonb) from public;
revoke all on function geo_tracker.citation_url_ids(jsonb) from anon;
revoke all on function geo_tracker.citation_url_ids(jsonb) from authenticated;
grant execute on function geo_tracker.citation_url_ids(jsonb) to service_role;

comment on function geo_tracker.citation_url_ids(jsonb) is
...
create or replace function geo_tracker.citations_domains(
  p_brand_id uuid,
  p_date_from timestamptz default null,
  p_date_to timestamptz default null,
  p_models text[] default null,
...
as $$
  with pairs as (
    ...
    from geo_tracker.prompt_result_citations c
    join geo_tracker.prompt_results pr on pr.id = c.prompt_result_id
    join geo_tracker.citation_urls cu on cu.id = c.url_id
    where c.brand_id = p_brand_id
      ...
      and (p_topic_ids  is null or pr.prompt_id in (
            select pp.id from geo_tracker.prompts pp where pp.topic_id = any(p_topic_ids)))
...
create or replace function geo_tracker.citations_urls(
  p_brand_id uuid,
  p_date_from timestamptz default null,
  p_date_to timestamptz default null,
  p_models text[] default null,
...
as $$
  ...
  join geo_tracker.citation_urls cu on cu.id = a.uid
  order by a.tc desc;
$$;
...
create or replace function geo_tracker.citations_window_stats(
  p_brand_id uuid,
  p_date_from timestamptz default null,
  p_date_to timestamptz default null,
  p_models text[] default null,
...
revoke all on function geo_tracker.citations_domains(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[]) from public;
revoke all on function geo_tracker.citations_urls(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[], integer) from public;
revoke all on function geo_tracker.citations_window_stats(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[]) from public;

grant execute on function geo_tracker.citations_domains(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[]) to authenticated, service_role;
grant execute on function geo_tracker.citations_urls(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[], integer) to authenticated, service_role;
grant execute on function geo_tracker.citations_window_stats(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[]) to authenticated, service_role;

comment on function geo_tracker.citations_domains(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[]) is
  'Per-domain citation aggregates for the Citations page (#732). Returns every domain — the long tail is the point.';
comment on function geo_tracker.citations_urls(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[], integer) is
...
comment on function geo_tracker.citations_window_stats(uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[]) is
...
DROP POLICY IF EXISTS "Users cannot update plan fields directly" ON "geo_tracker"."organizations";
DROP POLICY IF EXISTS "organizations: admin update" ON "geo_tracker"."organizations";
...
CREATE POLICY "organizations: admin update" ON "geo_tracker"."organizations"
  FOR UPDATE
  USING (
    "id" IN (
      ...
      FROM "geo_tracker"."profiles"
      WHERE "profiles"."id" = "auth"."uid"()
        AND "profiles"."role" = 'admin'::"geo_tracker"."user_role"
    )
  )
  WITH CHECK (
    "id" IN (
...
-- write, admin or not.
CREATE OR REPLACE FUNCTION "geo_tracker"."guard_organization_server_columns"()
RETURNS "trigger"
LANGUAGE "plpgsql"
SET "search_path" = ''
AS $$
...
DROP TRIGGER IF EXISTS "organizations_guard_server_columns" ON "geo_tracker"."organizations";

CREATE TRIGGER "organizations_guard_server_columns"
  BEFORE UPDATE ON "geo_tracker"."organizations"
  FOR EACH ROW
  EXECUTE FUNCTION "geo_tracker"."guard_organization_server_columns"();
...
-- not one per answer.
CREATE OR REPLACE FUNCTION "geo_tracker"."report_prompt_performance"(
  "p_brand_id" uuid,
  "p_date_from" timestamptz,
  "p_date_to" timestamptz
)
...
CREATE OR REPLACE FUNCTION "geo_tracker"."report_topic_performance"(
  "p_brand_id" uuid,
  "p_date_from" timestamptz,
  "p_date_to" timestamptz,
...
CREATE OR REPLACE FUNCTION "geo_tracker"."report_citation_evidence"(
  "p_brand_id" uuid,
  "p_date_from" timestamptz,
  "p_date_to" timestamptz,
...
GRANT EXECUTE ON FUNCTION "geo_tracker"."report_prompt_performance"(uuid, timestamptz, timestamptz) TO "authenticated";
GRANT EXECUTE ON FUNCTION "geo_tracker"."report_topic_performance"(uuid, timestamptz, timestamptz, timestamptz) TO "authenticated";
GRANT EXECUTE ON FUNCTION "geo_tracker"."report_citation_evidence"(uuid, timestamptz, timestamptz, integer, integer) TO "authenticated";
...
CREATE OR REPLACE FUNCTION "geo_tracker"."report_query_fanout"(
  "p_brand_id" uuid,
  "p_date_from" timestamptz,
  "p_date_to" timestamptz,
  "p_limit" integer DEFAULT 10
...
GRANT EXECUTE ON FUNCTION "geo_tracker"."report_query_fanout"(uuid, timestamptz, timestamptz, integer) TO "authenticated";
...
CREATE OR REPLACE FUNCTION "geo_tracker"."report_query_fanout_engines"(
  "p_brand_id" uuid,
  "p_date_from" timestamptz,
  "p_date_to" timestamptz
)
...
GRANT EXECUTE ON FUNCTION "geo_tracker"."report_query_fanout_engines"(uuid, timestamptz, timestamptz) TO "authenticated";
...
create table if not exists geo_tracker.insights_brand_daily (
  brand_id               uuid not null references geo_tracker.brands(id) on delete cascade,
  day                    date not null,
  model_used             text,
  platform               text,
...
create index if not exists idx_insights_brand_daily_brand_day
  on geo_tracker.insights_brand_daily (brand_id, day);
...
create table if not exists geo_tracker.insights_competitor_daily (
  brand_id         uuid not null references geo_tracker.brands(id) on delete cascade,
  day              date not null,
...
create index if not exists idx_insights_competitor_daily_brand_day
  on geo_tracker.insights_competitor_daily (brand_id, day);
...
create table if not exists geo_tracker.insights_prompt_daily (
  brand_id     uuid not null references geo_tracker.brands(id) on delete cascade,
  day          date not null,
  prompt_id    uuid not null,
  model_used   text,
...
create index if not exists idx_insights_prompt_daily_brand_day
  on geo_tracker.insights_prompt_daily (brand_id, day);
...
create table if not exists geo_tracker.insights_competitor_prompt_daily (
  brand_id      uuid not null references geo_tracker.brands(id) on delete cascade,
  day           date not null,
  competitor_id text not null,
  prompt_id     uuid not null,
...
create index if not exists idx_insights_competitor_prompt_daily_brand_day
  on geo_tracker.insights_competitor_prompt_daily (brand_id, day);

-- ─── RLS — same shape as prompt_results ─────────────────────────────────────

alter table geo_tracker.insights_brand_daily enable row level security;
alter table geo_tracker.insights_competitor_daily enable row level security;
alter table geo_tracker.insights_prompt_daily enable row level security;
alter table geo_tracker.insights_competitor_prompt_daily enable row level security;

create policy "Users can read own org insights rollups"
  on geo_tracker.insights_brand_daily for select
  using (brand_id in (
    select b.id from geo_tracker.brands b
    join geo_tracker.profiles p on p.organization_id = b.organization_id
    where p.id = auth.uid()));

create policy "Users can read own org competitor rollups"
  on geo_tracker.insights_competitor_daily for select
  using (brand_id in (
    select b.id from geo_tracker.brands b
    join geo_tracker.profiles p on p.organization_id = b.organization_id
    where p.id = auth.uid()));
...
create policy "Users can read own org prompt rollups"
  on geo_tracker.insights_prompt_daily for select
  using (brand_id in (
    select b.id from geo_tracker.brands b
    join geo_tracker.profiles p on p.organization_id = b.organization_id
    where p.id = auth.uid()));
...
create policy "Users can read own org competitor prompt rollups"
  on geo_tracker.insights_competitor_prompt_daily for select
  using (brand_id in (
    select b.id from geo_tracker.brands b
    join geo_tracker.profiles p on p.organization_id = b.organization_id
    where p.id = auth.uid()));
...
create or replace function geo_tracker.refresh_insights_daily(
  p_brand_id uuid,
  p_day_from date,
  p_day_to date
) returns void
...
as $$
begin
  delete from geo_tracker.insights_brand_daily
    where brand_id = p_brand_id and day between p_day_from and p_day_to;
  delete from geo_tracker.insights_competitor_daily
    where brand_id = p_brand_id and day between p_day_from and p_day_to;
  delete from geo_tracker.insights_prompt_daily
    where brand_id = p_brand_id and day between p_day_from and p_day_to;
  delete from geo_tracker.insights_competitor_prompt_daily
    where brand_id = p_brand_id and day between p_day_from and p_day_to;

  insert into geo_tracker.insights_brand_daily (
  ...
  insert into geo_tracker.insights_prompt_daily (
  ...
  insert into geo_tracker.insights_competitor_daily (
  ...
  insert into geo_tracker.insights_competitor_prompt_daily (
...
revoke execute on function geo_tracker.refresh_insights_daily(uuid, date, date)
...
create or replace function geo_tracker.insights_aggregates_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
...
as $$
  with filtered as (
    select * from geo_tracker.insights_brand_daily d
...
create or replace function geo_tracker.visible_prompt_stats_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
  ...
  select jsonb_build_object(
    'visible_prompts',
      (select count(distinct pd.prompt_id)
       from geo_tracker.insights_prompt_daily pd
    ...
    'visible_results',
      coalesce((select sum(bd.mentioning_answers)
       from geo_tracker.insights_brand_daily bd
...
create or replace function geo_tracker.tracked_prompt_count_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
...
create or replace function geo_tracker.ai_visibility_aggregates_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
...
as $$
  with brand_agg as (
    ...
    from geo_tracker.insights_brand_daily d
    where d.brand_id = p_brand_id
      and (p_platform is null or d.platform = p_platform)
      and (p_models is null or d.model_used = any (p_models))
      and (p_region is null or d.region = p_region)
  ...
  comp_agg as (
    ...
    from geo_tracker.insights_competitor_daily cd
    join geo_tracker.competitors c
...
create or replace function geo_tracker.competitor_aggregates_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
...
as $$
  ...
  prompt_days as (
    select * from geo_tracker.insights_prompt_daily d
    where d.brand_id = p_brand_id
      and (p_platform is null or d.platform = p_platform)
      and (p_models is null or d.model_used = any (p_models))
      and (p_region is null or d.region = p_region)
  ...
  comp_days as (
    select cd.* from geo_tracker.insights_competitor_daily cd
    join geo_tracker.competitors c
      on c.id::text = cd.competitor_id and c.brand_id = p_brand_id
    where cd.brand_id = p_brand_id
      and (p_platform is null or cd.platform = p_platform)
  ...
  comp_prompt_days as (
    select cpd.* from geo_tracker.insights_competitor_prompt_daily cpd
    join geo_tracker.competitors c
...
-- means keeping that.
create or replace function geo_tracker.share_of_voice_aggregates_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
...
as $$
  ...
  comp_days as (
    select * from geo_tracker.insights_competitor_daily d
    where d.brand_id = p_brand_id
      and (p_platform is null or d.platform = p_platform)
      and (p_models is null or d.model_used = any (p_models))
      and (p_region is null or d.region = p_region)
...
create or replace function geo_tracker.visibility_rate_trend_daily(
  p_brand_id uuid,
  p_platform text default null,
  p_models text[] default null,
  p_region text default null,
...
as $$
  ...
  prompt_daily as (
    ...
    from geo_tracker.insights_prompt_daily d
    where d.brand_id = p_brand_id
      and (p_platform is null or d.platform = p_platform)
      and (p_models is null or d.model_used = any (p_models))
      and (p_region is null or d.region = p_region)
  ...
  comp_visible as (
    ...
    from geo_tracker.insights_competitor_prompt_daily cpd
    join geo_tracker.competitors c
      on c.id::text = cpd.competitor_id and c.brand_id = p_brand_id
    where cpd.brand_id = p_brand_id
      and (p_platform is null or cpd.platform = p_platform)
...
create index if not exists prompt_result_citations_url_brand_idx
  on geo_tracker.prompt_result_citations (url_id, brand_id);

drop index if exists geo_tracker.prompt_result_citations_brand_created_idx;
drop index if exists geo_tracker.prompt_result_citations_url_idx;
...
create or replace function geo_tracker.citation_url_candidates(
  p_brand_id uuid,
  p_domain text
)
returns table (
...
as $$
  ...
  where cu.domain = p_domain
    and exists (
      select 1 from geo_tracker.prompt_result_citations c
...
-- largest brand.
create or replace function geo_tracker.citation_url_occurrences(
  p_brand_id uuid,
  p_url_ids bigint[],
  p_date_from timestamptz default null,
  p_date_to timestamptz default null,
...
as $$
  ...
  answers as (
    ...
    from hits h
    join geo_tracker.prompt_results pr on pr.id = h.rid
    where pr.brand_id = p_brand_id
      and pr.platform <> 'chatgpt-shopping'
      and (p_date_from  is null or pr.created_at >= p_date_from)
      and (p_date_to    is null or pr.created_at <= p_date_to)
  ...
  sizes as (
    ...
    from geo_tracker.prompt_result_citations a
  ...
  left join geo_tracker.prompts p on p.id = a.prompt_id
  order by a.created_at desc;
$$;

revoke all on function geo_tracker.citation_url_candidates(uuid, text) from public;
revoke all on function geo_tracker.citation_url_occurrences(uuid, bigint[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) from public;

grant execute on function geo_tracker.citation_url_candidates(uuid, text) to authenticated, service_role;
grant execute on function geo_tracker.citation_url_occurrences(uuid, bigint[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) to authenticated, service_role;

comment on function geo_tracker.citation_url_candidates(uuid, text) is
  'URLs on one domain that a brand has cited (#732), for the detail page to normalize and match in the app tier.';
comment on function geo_tracker.citation_url_occurrences(uuid, bigint[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) is
...
create or replace function geo_tracker.citation_gap_domains(
  p_brand_id uuid,
...
create or replace function geo_tracker.citation_competitor_sources(
  p_brand_id uuid,
  p_brand_domains text[],
  p_competitor_domains text[],
  p_date_from timestamptz default null,
...
revoke all on function geo_tracker.citation_gap_domains(uuid, text[], text[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) from public;
revoke all on function geo_tracker.citation_competitor_sources(uuid, text[], text[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) from public;

grant execute on function geo_tracker.citation_gap_domains(uuid, text[], text[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) to authenticated, service_role;
grant execute on function geo_tracker.citation_competitor_sources(uuid, text[], text[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) to authenticated, service_role;

comment on function geo_tracker.citation_gap_domains(uuid, text[], text[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) is
...
comment on function geo_tracker.citation_competitor_sources(uuid, text[], text[], timestamptz, timestamptz, text[], text[], uuid[], uuid[]) is
...
create or replace function geo_tracker.org_prompt_location_usage(p_organization_id uuid)
...
revoke all on function geo_tracker.org_prompt_location_usage(uuid) from public;
grant execute on function geo_tracker.org_prompt_location_usage(uuid) to authenticated, service_role;

comment on function geo_tracker.org_prompt_location_usage(uuid) is
...
from geo_tracker.prompt_sets ps, geo_tracker.brands b
...
comment on column geo_tracker.brands.state is
...
GRANT EXECUTE ON FUNCTION
  geo_tracker.prompt_visibility_summaries(uuid, timestamptz, timestamptz, text)
  TO authenticated;

COMMENT ON FUNCTION
  geo_tracker.prompt_visibility_summaries(uuid, timestamptz, timestamptz, text) IS
...
create or replace function geo_tracker.apply_prompt_locations(p_updates jsonb)
returns bigint
language sql
volatile
security invoker
...
revoke all on function geo_tracker.apply_prompt_locations(jsonb) from public;
grant execute on function geo_tracker.apply_prompt_locations(jsonb) to authenticated, service_role;

comment on function geo_tracker.apply_prompt_locations(jsonb) is
...
create or replace function geo_tracker.url_decode_safe(p_value text)
returns text
language plpgsql
immutable
parallel safe
...
comment on function geo_tracker.url_decode_safe(text) is
...
create or replace function geo_tracker.normalize_page_path(p_url text)
returns text
language sql
immutable
parallel safe
...
comment on function geo_tracker.normalize_page_path(text) is
...
-- the same 45 pages.
create or replace function geo_tracker.ga_page_ai_visibility(p_brand_id uuid, p_since date)
returns table (
  landing_page text,
  sessions bigint,
...
as $$
  ...
  cited as (
    ...
    from own_urls ou
    join geo_tracker.prompt_result_citations prc
      on prc.url_id = ou.id
     and prc.brand_id = p_brand_id
     and prc.created_at >= p_since
    join geo_tracker.prompt_results pr on pr.id = prc.prompt_result_id
  ...
  targeted as (
    ...
    select
      lower(geo_tracker.normalize_page_path(ptu.url)) as page_key,
      count(distinct ptu.prompt_id)::bigint as targeting_prompts
    from geo_tracker.prompt_target_urls ptu
    join geo_tracker.prompts p on p.id = ptu.prompt_id
    join geo_tracker.prompt_sets ps on ps.id = p.prompt_set_id
  ...
  pages as (
    select
      lower(geo_tracker.normalize_page_path(gps.landing_page)) as page_key,
      min(geo_tracker.normalize_page_path(gps.landing_page)) as landing_page,
    ...
    from geo_tracker.ga_page_stats gps
  ...
  ai as (
    select
      lower(geo_tracker.normalize_page_path(gat.landing_page)) as page_key,
      sum(gat.sessions)::bigint as ai_sessions,
      array_remove(array_agg(distinct gat.platform), null) as ai_platforms
    from geo_tracker.ga_ai_traffic_stats gat
...
revoke all on function geo_tracker.ga_page_ai_visibility(uuid, date) from public;
grant execute on function geo_tracker.ga_page_ai_visibility(uuid, date) to service_role;

comment on function geo_tracker.ga_page_ai_visibility(uuid, date) is
...
-- until then.
alter table geo_tracker.page_opportunities
...
comment on column geo_tracker.page_opportunities.citation_state is
...
drop function if exists geo_tracker.citations_urls(
...
as $$
  ...
  include_ids as (
    select cu.id from geo_tracker.citation_urls cu
...
revoke all on function geo_tracker.citations_urls(
  uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[], integer, text[], text[]
) from public;

grant execute on function geo_tracker.citations_urls(
  uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[], integer, text[], text[]
) to authenticated, service_role;

comment on function geo_tracker.citations_urls(
  uuid, timestamptz, timestamptz, text[], text[], uuid[], uuid[], integer, text[], text[]
) is
